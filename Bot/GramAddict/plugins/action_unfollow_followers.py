...
class ActionUnfollowFollowers(Plugin):
    ...
    def __init__(self):
        ...
        self.arguments = [
            ...
            {
                "arg": "--unfollow-skip-limit",
                "nargs": None,
                "help": "set the limit of skips while unfollowing",
                "metavar": "3",
                "default": "30",
            },
        ]
    ...
    def iterate_over_followings(
        ...
        for _ in range(2):
            ...
            if sort_container_obj.exists() and not has_been_sorted:
                has_been_sorted = self.sort_followings_by_date(
        ...
        total_skip_limit = get_value(
            self.args.unfollow_skip_limit, "Unfollow skip limit: {}", 30
        )
        hours_since_last_check = get_value(
            self.args.can_recheck_after, "Hours since last check limit: {}", 3
        )
        unfollow_delay = get_value(
            self.args.unfollow_delay, "Unfollow delay set to {} days", 0
        )
        skipped_users = 0
        job_iterated_users = []
        # screen_dimensions = device.get_info()["displayWidth"]
        list_view = device.find(
            resourceId=self.ResourceID.LIST,
        )
        # list_view_top = list_view.get_bounds()["top"] # TODO: implement this
        while True:
            if skipped_users >= total_skip_limit:
                logger.info("Reached skip limit. Stop unfollowing.")
                break
            start = time.time()
            ...
            for idx, item in enumerate(user_list):
                ...
                if following_status == FollowingStatus.UNFOLLOWED:
                    logger.warning(
                        f"You have already unfollowed @{username} on {interacted_when}. Probably you got a soft ban at some point."
                    )
                if filtered_when is not None and unfollow_restriction == [
                    UnfollowRestriction.ANY_NON_FOLLOWERS
                ]:
                    can_recheck = storage.can_be_rechecked(
                        filtered_when, hours_since_last_check
                    )
                    logger.info(
                        f"User {username} was already checked at {filtered_when.strftime('%H:%M:%S - %Y/%m/%d')}. {'Interacting again now' if can_recheck else 'Skip'}.",
                        extra={"color": f"{Fore.GREEN}"},
                    )
                ...
                if unfollow_restriction in [
                    ...
                    if following_status == FollowingStatus.NOT_IN_LIST:
                        ...
                        skipped_users += 1
                        continue
                    elif following_status == FollowingStatus.NONE:
                        logger.info(
                            f"@{username} has not been followed by this bot. Skip. (but it was in the list)"
                        )
                        skipped_users += 1
                        continue
                        ...
                        logger.info(
                            f"@{username} has been followed less then {unfollow_delay} days ago. Skip."
                        )
                        continue
            ...
            logger.debug(
                f"Iterated {len(screen_iterated_followings)} followings in {end - start:.2} seconds."
            )
